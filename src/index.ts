...
async function newsFeed(source:(typeof NEWS_FEEDS)[number]){try{const r=await fetch(source.url,{headers:{'user-agent':'market-pulse-readonly/1.3','accept':'application/rss+xml, application/xml, text/xml, */*'}});const t=await r.text();return{source:source.id,ok:r.ok,status:r.status,items:r.ok?parseNews(t,source):[] as NewsItem[]};}catch{return{source:source.id,ok:false,status:0,items:[] as NewsItem[]};}}
function has(t:string,words:string[]){const x=t.toLowerCase();return words.some(w=>x.includes(w));}
function newsRelevance(epic:string,t:string,source:string){let r=0;const macro=['federal reserve','fomc','interest rate','inflation','monetary policy','policy rate','central bank','dollar','usd','euro','ecb'];if(has(t,macro))r+=epic==='EURUSD'?70:45;if((epic==='GOLD'||epic==='SILVER')&&has(t,['gold','silver','precious metal','inflation','interest rate','dollar','usd']))r+=45;if((epic==='OIL_CRUDE'||epic==='OIL_BRENT')&&has(t,['oil','crude','brent','petroleum','energy','opec','production','supply','inventory']))r+=75;if(source.startsWith('FED')&&['EURUSD','GOLD','SILVER'].includes(epic))r+=20;if(source==='ECB_MID'&&epic==='EURUSD')r+=25;return Math.min(100,r);}
function newsDirection(epic:string,t:string){let macro=0,oil=0;const hawkish=['rate hike','raises rates','higher rates','restrictive','inflation remains elevated','inflation elevated','tightening'];const dovish=['rate cut','cuts rates','lower rates','easing','reduce the target range','lower the target range'];if(has(t,hawkish))macro+=1;if(has(t,dovish))macro-=1;if(has(t,['production cut','output cut','supply disruption','supply shortage','lower production','decline in inventories']))oil+=1;if(has(t,['production increase','increase production','higher production','inventory build','rise in inventories','supply increase']))oil-=1;if(epic==='EURUSD'||epic==='GOLD'||epic==='SILVER') return macro===0?0:(macro>0?-1:1);if(epic==='OIL_CRUDE'||epic==='OIL_BRENT')return oil===0?0:(oil>0?1:-1);return 0;}
function newsForEpic(epic:string,items:NewsItem[]){const classified=items.map(item=>{const relevance=newsRelevance(epic,item.text,item.source_id);const direction=newsDirection(epic,item.text);const age=item.age_minutes;const decay=age===null||age>1440?0:Math.exp(-age/180);const impact=Math.min(100,25+relevance*.55+(direction?20:0));const signed=direction*relevance/100*impact/100*decay*100;return{source:item.source_name,title:item.title,url:item.url,published_at:item.published_at,age_minutes:age===null?null:Math.round(age),relevance:Math.round(relevance),impact:Math.round(impact),direction:direction>0?'BULLISH':direction<0?'BEARISH':'NEUTRAL',signed_score:Math.round(signed*100)/100,active:relevance>=35&&decay>0};}).sort((a,b)=>Math.abs(b.signed_score)-Math.abs(a.signed_score)||b.relevance-a.relevance);const active=classified.filter(x=>x.active);const denom=active.reduce((s,x)=>s+Math.max(1,x.relevance),0);const score=denom?active.reduce((s,x)=>s+x.signed_score*Math.max(1,x.relevance),0)/denom:0;return{epic,items_considered:classified.length,active_items:active.length,signed_score:Math.round(score*100)/100,bias:score>=5?'BULLISH':score<=-5?'BEARISH':'NEUTRAL',top_items:classified.slice(0,8)};}
async function newsEngine(epic:string){if(!WATCHLIST.some(x=>x.epic===epic))throw new Fault('EPIC_NOT_ALLOWED',400);const results=await Promise.all(NEWS_FEEDS.map(newsFeed));const seen=new Set<string>(),items:NewsItem[]=[];for(const x of results)for(const item of x.items){const k=item.url||item.id;if(!seen.has(k)){seen.add(k);items.push(item);}}return{success:results.some(x=>x.ok),...INFO,module:'NEWS_MACRO_ENGINE',epic,fetched_at:new Date().toISOString(),source_health:{configured:results.length,working:results.filter(x=>x.ok).length,failed:results.filter(x=>!x.ok).length,sources:results.map(x=>({source:x.source,ok:x.ok,status:x.status,items:x.items.length}))},news:newsForEpic(epic,items),trading:'DISABLED',execution:'NONE',note:'Official-source RSS research classifier. Direction is deterministic context, not a prediction or trade instruction.'};}
...
<header><div><h1>Market Pulse</h1><small>V1.3.0 · Capital.com · Signal + News/Macro</small></div><span class="badge">DEMO · READ ONLY</span></header>
